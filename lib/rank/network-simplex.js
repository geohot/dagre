...
function dfsAssignLowLim(tree, visited, nextLim, startV, startParent) {
  // Iterative post-order DFS over the tree
  let callStack = [[startV, startParent, 0, nextLim]]; // [node, parent, childIdx, low]
  while (callStack.length > 0) {
    ...
    for (let i = childIdx; i < neighbors.length; i++) {
      ...
      if (!Object.hasOwn(visited, w)) {
        frame[2] = i + 1; // update childIdx for when we return
    ...
    if (!pushed) {
      // All children processed, assign low/lim
